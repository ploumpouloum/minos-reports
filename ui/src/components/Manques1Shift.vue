...
const shiftManques = computed(() => { return main.getManques(props.shiftId)})
...
<template>
  <tr v-if="shift && station">
    ...
    <td class="manques" v-if="Object.keys(shiftManques).length > 0">
      <span v-for="(count, role, idx) in shiftManques" :key="role">{{ idx ? ', ' : 'Manque '}}<b>{{ count }}</b> {{ role }}</span>
    </td>
    <td class="manques complet" v-else>
      COMPLET
    </td>
  </tr>
  <tr v-else>Missing data</tr>
</template>
